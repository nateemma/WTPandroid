...
public class CoolingLiquidsResultsActivity extends Activity {
	...
	private static final String TAG = "CoolingLiquidsResultsActivity";
	...
	private String rowText(int nameId, Double dosage, Double usage, int descriptionId){
		String row = "";
		try{
			String name = mContext.getResources().getString(nameId);
			if (name==null){
				Log.w(TAG, "rowtext: name is null !");
			}
			String description = mContext.getResources().getString(descriptionId);
			if (description==null){
				Log.w(TAG, "rowtext: description is null !");
			}			
			row = "<TR><TD>" + name + "<TD>" + Utilities.round1(dosage)  + "<TD>" + Utilities.round1(usage)  + "<TD>" + description + "</TR>" ;
		} catch (Exception e){
